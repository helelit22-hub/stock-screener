...
def analyze_portfolio(holdings):
    ...
    for h in holdings:
        ...
        try:
            ...
            prev_ma150 = float(d['Close'].rolling(150).mean().iloc[-2]) if len(d) >= 151 else None
            ...
            if ma150 is not None:
                if price > ma150:
                    ...
                    flags.append('ℹ️ מתחת MA150')
                # התראה רק כששובר היום (אתמול היה מעל, היום מתחת) — לא אם כבר מתחת
                if (prev_ma150 is not None
                        and prev_close >= prev_ma150
                        and price < ma150):
                    alerts.append(f'{t} שבר היום את ה-MA150 (${price:.2f} מתחת ל-${ma150:.2f})')
            if atr is not None:
                if 3.5 <= atr <= 7.0:
