    ...
    for h in holdings:
        ...
        try:
            ...
            if ma150 is not None:
                flags.append('✅ מעל MA150' if price > ma150 else '⚠️ מתחת MA150')
            if atr is not None:
                if 3.5 <= atr <= 7.0:
...
def save_excel(final, portfolio, output_dir, ts):
    ...
    port_cols = ['Ticker', 'Entry', 'Price', 'Gain_$', 'Gain_%', 'MA150', 'vs_MA_%', 'ATR14', 'Status']
...
def build_email_html(final, tickers, now, portfolio=None):
    rows_html = ""
    for s in final[:30]:
...
if __name__ == "__main__":
    ...
    html = build_email_html(final, tickers, now, portfolio=portfolio)
    try:
